...
button_event[TP_CDP, 0] {
	push: {
		...
		switch (index_button) {
			...
			/* -------------------------------------------------------------------------- */
			case 201: {
				cdp_send(DV_CDP_02, CDP_CMD_STANDBY)
			}
			case 202: {
				cdp_send(DV_CDP_02, CDP_CMD_POWERON)
			}
			case 203: {
				cdp_send(DV_CDP_02, CDP_CMD_STOP)
				tp_on_btn_ss(TP_CDP, 203)
				tp_off_btn_ss(TP_CDP, 204)
			}
			case 204: {
				cdp_send(DV_CDP_02, CDP_CMD_PLAY)
				tp_on_btn_ss(TP_CDP, 204)
				tp_off_btn_ss(TP_CDP, 203)
			}
			case 205: {
				cdp_send(DV_CDP_02, CDP_CMD_PAUSEPLAY)
			}
			case 206: {
				cdp_send(DV_CDP_02, CDP_CMD_SKIPBACK)
			}
			case 207: {
				cdp_send(DV_CDP_02, CDP_CMD_CUE)
			}
			case 208: {
				cdp_send(DV_CDP_02, CDP_CMD_FASTFORWARD)
			}
			case 209: {
				cdp_send(DV_CDP_02, CDP_CMD_REWIND)
			}
			case 210: {
				cdp_send(DV_CDP_02, CDP_CMD_TRACKJUMPNEXT)
			}
			case 211: {
				cdp_send(DV_CDP_02, CDP_CMD_TRACKJUMPPREV)
			}
			case 212: {
				cdp_send(DV_CDP_02, CDP_CMD_RECORD)
			}
			case 213: {
				cdp_send(DV_CDP_02, CDP_CMD_PAUSERECORD)
			}
		}
	}
}
